...
export function useSetupMutation() {
  ...
  const mutation = useMutation({
    mutationFn: async (payload: SavePayload) => {
      const current =
        queryClient.getQueryData<SetupDTO | null>(setupQueryOptions.queryKey) ??
        null;
    ...
    onMutate: async (payload) => {
      ...
      const previous = queryClient.getQueryData<SetupDTO | null>(
        setupQueryOptions.queryKey,
      );
      ...
      const optimistic: SetupDTO = {
        ...
        styles: payload.styles ?? base.styles,
        notifications: payload.notifications ?? base.notifications,
        completedAt: payload.markCompleted
          ? new Date().toISOString()
          : base.completedAt,
      };
      queryClient.setQueryData<SetupDTO | null>(
        setupQueryOptions.queryKey,
        optimistic,
      );
      return { previous };
  ...
  const save = useCallback(
    (payload: SavePayload) => mutation.mutate(payload),
    [mutation],
  );
