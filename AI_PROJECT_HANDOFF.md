...
| Signals dashboard | `/dashboard` | `routes/_authenticated/dashboard.tsx`, `components/BrandCard.tsx` | Backend-connected | Reads `listHousesForDashboard`; sort uses personal setup + style affinity. Client-side pagination (12/page) over the filtered+sorted list; page state in `?page` URL param via `validateSearch`; filter changes reset to page 1; out-of-range pages are clamped. |
...
| Watchlist | `/watchlist` | `routes/_authenticated/watchlist.tsx`, `data/store.ts`, `lib/watchlist.functions.ts`, `user_watchlist` table | Production-ready, optimistic | Recently fixed toast loop; do not re-add effect-driven popups. URL-backed search (`?q=`), category filter (`?cat=`), and pagination (`?page=`, 12/page) via `validateSearch`; sort still in localStorage; department filter still synced via `setup`. Empty-filter state offers a "Clear search & filters" reset; out-of-range pages are clamped. |
