...
- **Scroll restoration**: router uses `scrollRestoration: true` with `getScrollRestorationKey: (loc) => loc.pathname` so `/dashboard` and `/watchlist` (and every other route) restore the exact prior scroll position on back/forward AND on re-entry via the top-nav link. Cross-route navigation still scrolls to top because each pathname is its own key. Window is the scroll container (no internal `overflow-auto` on `PageLayout`/`main`), so default window scroll restoration applies — do not add an `overflow-y` scroll container to the app shell or this breaks. **In-place refinements** (watchlist search/category `navigate({ search, replace: true })`) pass `resetScroll: false` so updating the URL search params keeps the user's scroll where it is; only true route changes scroll to top. Add/remove watchlist mutations are pure state changes (no navigation) and naturally preserve scroll.
- **Focus restoration** (`src/lib/focus-restoration.ts`, wired in `__root.tsx` `RootComponent`): on every router navigation, snapshots `document.activeElement`'s selector against the current `history.state.key` in `sessionStorage`; on `onResolved`, restores focus to the saved selector for the new history entry. Means keyboard / screen-reader users who tab to a link, navigate forward, then come back land on the same control. Selector preference: `data-focus-key` → `id` → short `tag:nth-of-type(n)` path capped at 6 levels. Restoration uses `focus({ preventScroll: true })` and double-RAF so it doesn't fight scroll restoration or any in-route auto-focus. Add `data-focus-key="…"` to bespoke controls that re-render across navigation if structural selectors prove brittle.
- **Patterns**:
...
- **DB-driven theming (Jun 2026)**: every recurrent visual variable is registered in `src/lib/theme/registry.ts` (key, cssVar, label, description, group, type, default). Per-theme seed values live alongside as `SEEDED_THEME_OVERRIDES` / `seededDefaultsFor(themeKey)` — Editorial uses registry defaults, Playful lists its overrides, duplicates fall back to registry. Themes live in the `themes` table (`key`, `name`, `tokens jsonb`, `is_active`; partial unique index ensures one active row). The root route (`__root.tsx`) calls `getActiveTheme()` in its `loader` and injects the result as `<style id="theme-tokens">:root{…}</style>` after `<HeadContent />`, so DB tokens override the static `styles.css` defaults during SSR — no FOUC and logged-out pages stay themed. Server fns in `src/lib/theme.functions.ts`: `getActiveTheme` (public read via publishable-key client), `listThemes` / `getTheme` / `upsertThemeTokens` / `createTheme` / `setActiveTheme` (admin via `requireSupabaseAuth` + `has_role`; `setActiveTheme` calls the `public.set_active_theme` SECURITY DEFINER RPC for atomic flip). Structural tokens `--radius-card/-button/-badge`, `--border-width`, `--label-transform`, `--label-tracking` plus shared utilities `ui-card-radius`, `btn-shape`, `badge-shape`, `tab-trigger`, `label-style` mean themes can re-shape corners and label casing, not just colour. Admin UI: `src/components/admin/ThemeTab.tsx` under Admin → Sales → **Design** tab (registry-driven grouped form, theme picker, Set-as-active + Duplicate, live preview toggle, Revert, **Reset to <theme> defaults** that resets to the seeded values of whichever theme is being edited; per-token Reset button + "Changed" dot + inline before/after values relative to that theme's seeded default; inline "Saved" — no toasts). Two seeded themes: **Editorial** (active, mirrors current look exactly) and **Playful** (rounded, vibrant, sentence-case labels).
...
| 13 | Done | Loader-error overlays on dashboard↔brand navigation (root `errorComponent` over View-Transition snapshots) | Dashboard/watchlist treated optional `setup` prefetch as critical; brand loader called `getHouseDetail` with no retry; brand loader's catch did not distinguish router-cancelled aborts | Split critical vs optional prefetches in dashboard/watchlist (only `setup` is background). Wrapped `getHouseDetail` in `houseDetailQueryOptions(slug)` (`src/lib/brands.functions.ts`) with `retry: 2`, exponential backoff (400ms→1.5s). Brand loader uses `ensureQueryData(houseDetailQueryOptions(...))` and re-throws when `abortController.signal.aborted`. All three routes (brand, dashboard, watchlist) render a shared `<RouteErrorCard>` (`src/components/RouteErrorCard.tsx`) which maps raw error messages to friendly editorial copy (network/timeout/auth/5xx), wraps in `<PageLayout>`, has `role="alert"`, `aria-live="polite"`, and auto-focuses the retry button (which calls `router.invalidate()` + `reset()`). Temporary `console.warn("[*-loader] non-fatal failure", err)` lines in each boundary — **remove after verification in deployed preview** | Done (verify in prod, then strip warns) |
