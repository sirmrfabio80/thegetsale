...
| 13 | Done | Loader-error overlays on dashboard↔brand navigation (root `errorComponent` over View-Transition snapshots) | Dashboard/watchlist treated optional `setup` prefetch as critical (any failure tore down the route); brand loader called `getHouseDetail` with no retry so a single transient failure surfaced an error UI; brand loader's catch did not distinguish router-cancelled aborts from real failures | Split critical vs optional prefetches in dashboard/watchlist (only `setup` is background, awaited via `void … .catch(() => {})`); added in-page `errorComponent` + `notFoundComponent` on both, using `router.invalidate()` + `reset()` to retry. Wrapped `getHouseDetail` in `houseDetailQueryOptions(slug)` (`src/lib/brands.functions.ts`) with `retry: 2`, exponential backoff (400ms→1.5s). Brand loader now calls `ensureQueryData(houseDetailQueryOptions(...))` and re-throws when `abortController.signal.aborted`. Temporary `console.warn("[*-loader] non-fatal failure", err)` lines in all three boundaries to distinguish sandbox HMR noise from real production failures — **remove after verification in deployed preview** | Done (verify in prod, then strip warns) |
